...
export function SecretSantaRoomClient({ initialRoom }: { initialRoom: Room }) {
  ...
  const [identity, setIdentity] = useState<{ playerId: string; playerToken: string } | null>(null);
  ...
  useEffect(() => {
    ...
    upsertGameHistory({ code: room.code, gameType: "secret_santa", playerId: identity.playerId, playerToken: identity.playerToken, playerName: me.name, role: me.isHost ? "host" : "participant", status: room.status === "active" ? "complete" : "lobby", playerCount: room.players.length, ...(room.status === "active" ? { completedAt: new Date().toISOString() } : {}) });
  }, [identity, me, room]);
  ...
  async function addManual() {
    ...
    const result = await addManualPlayers(room.code, [manualName], identity.playerId, identity.playerToken);
    setMessage(result.ok ? `${manualName} added ✓` : result.error || "Could not add player");
  ...
  return (
    <div className="space-y-5 rounded-3xl border-2 border-black bg-white p-4 shadow-[4px_4px_0_#000] sm:p-7">
      ...
      <section className="rounded-2xl bg-kringle-spruce/5 p-3 text-center">
        <p className="text-[10px] font-black uppercase tracking-widest text-kringle-spruce">Room code</p><p className="font-mono text-4xl font-black tracking-[.16em] text-kringle-spruce">{room.code}</p>
        {showQr && <div className="mx-auto mt-3 w-fit rounded-xl bg-white p-2"><QRCode value={inviteUrl} size={116} fgColor="#0f5132" /></div>}
        <p className="mt-1 text-xs font-semibold text-kringle-spruce/70">Invite people, then draw when everyone is ready.</p>
        <div className="mt-3 grid grid-cols-3 gap-2"><button onClick={() => navigator.share?.({ text: shareText, url: inviteUrl })} className="min-h-10 rounded-xl bg-kringle-spruce text-xs font-black text-white">Share</button><button onClick={() => setShowQr((value) => !value)} className="min-h-10 rounded-xl border-2 border-kringle-spruce text-xs font-black text-kringle-spruce">{showQr ? "Hide QR" : "Show QR"}</button><button onClick={() => navigator.clipboard.writeText(shareText).then(() => setMessage("Invite copied ✓"))} className="min-h-10 rounded-xl border-2 border-kringle-spruce text-xs font-black text-kringle-spruce">Copy link</button></div>
      </section>
